...
class news_search:
    def news():
        ...
        newsapi = NewsApiClient(api_key='9727b7bd986942c3ae794e16bc41b642')
        flag = -1   
        while flag == -1:    
            try:
                ...
                if Headlines:
                        for articles in Headlines:
                            ...
                            if "news" in (articles['title'][-b+1:]).lower():
                                print(
                                    f"\n{articles['title'][-b+1:]}: {articles['title'][:-b-2]} link: {articles['url']}")
                            else:
                                print(
                                    f"\n{articles['title'][-b+1:]} News: {articles['title'][:-b-2]} link : {articles['url']}")
                else:
                    print(
                        f"Sorry no articles found for {input_country}, Something Wrong!!!")
                flag = 0
            except:
                print('Wrong country name, please enter correctly!')
